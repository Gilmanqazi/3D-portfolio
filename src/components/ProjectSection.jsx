...
panels.forEach((panel, i )=>{
  ...
  //create a timlinew for each panerl
  ...
  //iamge scale and opacity animabtion

  t1.fromTo(image ,{scale:0, rotate:-20 }, {scale:1, rotate:1, duration:0.5})

  //Titlke animation if it exists
  if(imageTitle){
...
{/* Section title */}
<div className='container mx-auto px-4 mb-16 reltaive z-10  '>
<h2 ref={titleRef} className='text-4xl md:text-5xl font-bold text-black text-center mb-4 opacity-0'>Featured Projects</h2>
<div ref={titleLineRef} className='w-0 h-1 bg-gradient-to-r from-purple-500 to-pink-500 mx-auto opacity-0'>

</div>
</div>

{/* Hoeizontal Scroll Section */}
...
{projectImages.map((project)=>(
  ...
  className='panel relative flex items-center justify-center'>
    ...
    md:p-12'>
    <img className='project-image max-w-full max-h-full rounded-2xl object-cover' src={project.imageSrc} alt="" />
    
    <h2 className='project-title flex items-center gap-3 md:text-3xl text-sm md:font-bold text-black mt-6 z-50 text-nowrap hover:text-gray-400 transition-colors duration-300 cursor-pointer'>{project.title}</h2>
